...
#! FILE INPUT REQUIRED !#
dir = 'Path to /fileListBlue.txt'
baseData = fits.open('/Path to /GHOSTDR/ghostdr/ghost/lookups/Polyfit/blue/high/220620/wavemod.fits')
...
pp1 = PdfPages(f'chipShiftBlue1.pdf')
...
for i, (date, flat, arc, resid) in enumerate(zip(dateList[1:], flatList[1:], arcList[1:], residList[1:]), start=1): 
                ...
                if line in commonLineList: 
                    ...
                    if (arc1Range == None).any() or (arc2Range == None).any() or len(arc1Range[:,0]) < 5:
                        ...
                        try:
                            ...
                            waveParams1, p_covw1 = curve_fit(gauss, arc1WaveData, arc1FluxData, p0=((arc1MaxFlux, arc1MaxWave, 0.1)), maxfev=10000)
                            waveParams2, p_covw2 = curve_fit(gauss, arc2WaveData, arc2FluxData, p0=((arc2MaxFlux, arc2MaxWave, 0.1)), maxfev=10000)
                        except RuntimeError:
    ...
    ax1.set_title(f'{baseDate} {arcDate} Arc Blue1 Comparison\n(Subtracted Mean Difference)')
...
for i in range(6):
    for j in range(6):
        ...
        plot1 = ax1.scatter(compareArr[:,0], compareArr[:,i+j+1], c='b')
        ax1.set_title(f'Q{i}{j} Coefficient Difference vs Pixel Shift (Blue)')
