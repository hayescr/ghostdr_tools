#This program will allow you to check the fit of the lines for a particular RED arc file
...
pp1 = PdfPages(f'curveFitting_{arcName}_red.pdf')
...
for index, wavelength in enumerate(wavelengths):
    ...
    for i in range(arcData[1].data.shape[0]):
        ...
        wave = poly_arc_model(yprimes- 6144//2)
        xval = poly_xmod_model(yprimes- 6144//2) + 6160//2
...
for i in range(plotArr.shape[0]):
    ...
    if plotArr[i,4] < 0.1:
        ...
        if avgFWHM-2*fwhmSTD < plotArr[i,3] < avgFWHM+3.5*fwhmSTD:
            #Checks if the fit error is around the average
            if avgFitError > fitErrorSTD:
                if plotArr[i,4] <= avgFitError+3.5*fitErrorSTD:    
                    filteredData.append([plotArr[i,0], plotArr[i,1], plotArr[i,2], plotArr[i,3], plotArr[i,4], plotArr[i,0]/plotArr[i,3]])
                else:
                ...
                if plotArr[i,4] <= avgFitError+2.5*fitErrorSTD:    
                    filteredData.append([plotArr[i,0], plotArr[i,1], plotArr[i,2], plotArr[i,3], plotArr[i,4], plotArr[i,0]/plotArr[i,3]])
                else:
                    flagged.append([plotArr[i,0], plotArr[i,1], plotArr[i,2], plotArr[i,3], plotArr[i,4], plotArr[i,0]/plotArr[i,3],  "Flag 2"])
                    print(f"{plotArr[i,0]} was flagged! (FLG4)")
...
with pd.ExcelWriter(f'comparePlotChip_{arcName}_red.xlsx') as writer:
